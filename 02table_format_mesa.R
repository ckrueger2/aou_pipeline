...
if (check_result2 != 0) {
  ...
  cat("Reference TOPMed MESA file successfully transferred to bucket.\n")
}
...
#rename header
mesa_table$"#CHROM" <- mesa_table$CHR
mesa_table$CHR <- NULL
...
cat("Final TOPMed MESA filtered table:\n")
head(mesa_table)
...
write.table(mesa_table, mesa_destination_filename, col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
...
#MESA file
check_gtex <- system(paste0("gsutil ls ", my_bucket, "/data/ | grep ", mesa_destination_filename), ignore.stderr = TRUE)

if (check_mesa != 0) {
  stop(paste0("ERROR: File '", mesa_destination_filename, "' was not found in bucket ", my_bucket, "/data/"))
} else {
  cat("TOPMed MESA formatted file successfully saved to bucket.\n")
}
